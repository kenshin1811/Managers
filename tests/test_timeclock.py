...
def test_logged_hours_take_somebody_out_of_the_candidate_pool(session, settings, short_staffed):
    luis_id = short_staffed.employee_id("luis")
    task = short_staffed.tasks["pack_1043"]
    now = at(14, 5)
    ...
    def is_eligible() -> bool:
        ...
        ranked = ranking.rank_candidates(
            ...
            window_end=at(14, 45),
            critical=True,
            vacating_employee_id=short_staffed.employee_id("mai"),
        )
        return next(c for c in ranked if c.employee_id == luis_id).eligible

    assert is_eligible(), "Luis starts out available"
...
def test_last_weeks_hours_do_not_count_against_this_week(session, settings, short_staffed):
    luis_id = short_staffed.employee_id("luis")
    for offset in range(8, 12):
        day = SIM_DATE - timedelta(days=offset)
        session.add(
            TimeEntry(employee_id=luis_id, clock_in=at(8, 0, day=day), clock_out=at(20, 0, day=day))
        )
    session.flush()

    snapshot = load_snapshot(session, settings, at(14, 5))
    assert snapshot.get(luis_id).minutes_week == pytest.approx(0.0)
